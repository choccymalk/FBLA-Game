...
public class BusinessMiniGame {
    ...
    private String[] playerChoices = {"DEFAULT", "DEFAULT", "DEFAULT"};
    ...
    private boolean allScreensDisplayed = false;
    
    public BusinessMiniGame(main game, ImGuiImplGl3 imguiGl3, GameRenderer renderer) {
        this.game = game;
        this.imguiGl3 = imguiGl3;
        this.screen0Tex = renderer.loadTexture(game.RESOURCE_PATH + "\\textures\\OfficeGameAudience2.png.png");
        this.screen1Tex = renderer.loadTexture(game.RESOURCE_PATH + "\\textures\\OfficeGameProduct2.png");
    ...
    public void render(int winW, int winH) {
        ...
        if(currentScreen == 0){
            ...
            ImGui.setCursorPos(245, 530);
            if(ImGui.button(screen0Choices[0], 200, 80)){
                playerChoices[currentScreen] = screen0Choices[0];
                currentScreen++;
            }
            ImGui.setCursorPos(525, 530);
            if(ImGui.button(screen0Choices[1], 200, 80)){
                playerChoices[currentScreen] = screen0Choices[1];
                currentScreen++;
            }
            ImGui.setCursorPos(810, 530);
            if(ImGui.button(screen0Choices[2], 200, 80)){
        ...
        if(currentScreen == 1){
            ...
            ImGui.setCursorPos(245, 530);
            if(ImGui.button(screen1Choices[0], 200, 80)){
                playerChoices[currentScreen] = screen1Choices[0];
                currentScreen++;
            }
            ImGui.setCursorPos(525, 530);
            if(ImGui.button(screen1Choices[1], 200, 80)){
                playerChoices[currentScreen] = screen1Choices[1];
                currentScreen++;
            }
            ImGui.setCursorPos(810, 530);
            if(ImGui.button(screen1Choices[2], 200, 80)){
        ...
        if(currentScreen == 2){
            ...
            ImGui.setCursorPos(245, 530);
            if(ImGui.button(screen2Choices[0], 200, 80)){
                playerChoices[currentScreen] = screen2Choices[0];
                currentScreen++;
            }
            ImGui.setCursorPos(525, 530);
            if(ImGui.button(screen2Choices[1], 200, 80)){
                playerChoices[currentScreen] = screen2Choices[1];
                currentScreen++;
            }
            ImGui.setCursorPos(810, 530);
            if(ImGui.button(screen2Choices[2], 200, 80)){
